...
- [x] 9.6 Playwright e2e (`frontend/e2e/`): register→complete lesson→earn XP, and login→Play lobby. **Both pass live against :8080**; wired into CI as its own job. **2026-07-26: fixed the CI e2e job — it had never passed** (exit 243, `EACCES` on `frontend/node_modules`). Two causes: (1) the compose `frontend` service bind-mounts `frontend/` and runs `npm install` as root → added an anonymous `/app/node_modules` volume so container and host keep separate trees; (2) Docker creates that mountpoint as root anyway, so `npm ci` + `playwright install` now run **before** `compose up`. **CI green across all 5 jobs.**
